...
Todas as transformações e processamentos descritos nesta subseção foram realizados por meio do notebook "Network_data.ipynb", disponível no diretório \notebooks. Ao final deste notebook, realizamos também algumas análises básicas da rede de condições psiquiátricas: valores máximo e mínimo de _log odds_, quantidade de arestas com _odds ration_ maior que 1.5 (equivalente a um _log odds_ maior que 0.405) e a quantidade dessas arestas que não são recíprocas.
...
Para além das comunidades, notamos, sem grandes surpresas, que as próprias arestas que utilizamos para melhor visualização da rede diferiram um pouco daquelas existentes nas redes apresentadas pelo artigo. Talvez um dos principais motivos para essas diferenças seja o fato de termos utilizado uma árvore geradora máxima como base enquanto o artigo usou uma árvore geradora mínima. De fato, não compreendemos muito bem porque utilizar a abordagem mínima nesse caso (como comentado anteriormente) e vale destacar que o artigo não entrou em muitos detalhes sobre como essa árvore foi obtida ou qual o algoritmo utilizado, por exemplo.
